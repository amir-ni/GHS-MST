...
class ConnectRequests(object):
    ...
    def get_least_level_req(self):
        if self.requests:
            return min(self.requests, key=self.requests.get)
...
class MSTNode(threading.Thread):
    def __init__(self, uid, port, edges, timeout):
        ...
        self.other_core_node = None  # set to other core node in merge proccess
    ...
    def run(self):
        ...
        self.node_listener()  # start a listen proccess for node port to receive messages
    ...
    def node_sender(self, message, edge):
        ...
        except:  # happens if dest node listener has stopped
            pass

    # create message and pass it to message channel
    def send_message(self, type, value, edge):
        start_new_thread(  # create instance of message transfer
            self.node_sender,
            (
                Message(self.uid, type, value),
                edge
            ),
        )
    ...
    def getEdgeById(self, id):
        return next(x for x in self.edges if int(x.dest_node) == int(id))

    # changes node state and sends connect message to minimum weight outgoing edge
    def WakeUp(self):
        if self.state == SLEEP:
            self.state = FOUND
            min_weight_outgoing_edge = min(self.edges, key=lambda x: x.weight)
            self.send_message('Connect', str(self.core) +
                              ',' + str(self.level), min_weight_outgoing_edge)
        ...
        while self.active:
            try:
                ...
                if m.type == "End":
                    ...
                    start_new_thread(
                        self.do_connect, (m.uid, data[0], int(data[1])))  # Core, Level
                elif m.type == "Test":
                    data = str(m.value).split(',')
                    start_new_thread(
                        self.do_test, (m.uid, int(data[0]), int(data[1])))  # Core, Level
                elif m.type == "Initiate":
                    data = str(m.value).split(',')
                    start_new_thread(self.do_initiate, (m.uid, int(  # Core, Level, State, merge
                        data[0]), int(data[1]), data[2], bool(data[3])))
                elif m.type == "Reject":
                    ...
                    if(data[1] != '' and data[1]):
                        data[1] = data[1].split('::')
                    start_new_thread(
                        self.do_report, (m.uid, int(data[0]), data[1]))
    ...
    def do_end(self, timeout):
        time.sleep(timeout)
        self.active = False
        branches = ",".join(str(edge.dest_node)
                            for edge in self.edges if edge.state == BRANCH)
    ...
    def do_initiate(self, Sender, Core, Level, State, merge):
        ...
        my_branches = {edge for edge in self.edges if edge.state ==
                       BRANCH} - {self.getEdgeById(Sender)}

        self.find_count = 0
        for b in my_branches:
            self.send_message('Initiate', str(
                self.core) + ',' + str(self.level) + ',' + str(self.state) + ',' + str(False), b)
    ...
    def do_change_root(self, Sender, path):
        if len(path) == 0:
            node = Sender
            self.send_message('Connect', str(self.core) +
                              ',' + str(self.level), self.getEdgeById(node))
            ...
            hd = path[1]
            tl = path[2:]
            self.send_message('ChangeRoot', str('::'.join(str(v)
                                                          for v in tl)), self.getEdgeById(hd))

    # happens if no one is remaining to send test message and got response from all search requests sent
    def report(self):
        self.test_over = None
        self.state = FOUND
        self.send_message('Report', str(self.best_weight) + ',' + str('::'.join(str(node)
                                                                                for node in self.best_path)), self.getEdgeById(self.find_source))
    ...
    def merge(self, node):
        ...
        self.send_message('Initiate', str(new_core) + ',' +
                          str(self.level + 1) + ',' + str(SEARCH) + ',' + str(True), connection_edge)  # initiate with merge=True
        self.expect_core_report = True
        self.discovery = False

    # happens if received connect from someone with lower level
    def absorb_node(self, node):
        self.send_message('Initiate', str(self.core) + ',' + str(self.level) +
                          ',' + str(self.state) + ',' + str(False), self.getEdgeById(node))
        self.getEdgeById(node).state = BRANCH
    ...
    def test(self):
        ...
        if basic_edges:
            ...
            self.send_message('Test', str(self.core) + ',' +
                              str(self.level), self.test_edge)
        else:
            self.test_over = True
    ...
    def process_test_requests(self):
        ...
        for (L, F, j) in self.test_requests:
            ...
            # if diffrent fragment and level in message < self.level -> an accept message is sent back.
            elif L <= self.level:
    ...
    def main(self):
        while self.active:
            ...
            # if diffrent fragment and level in message < self.level –> no reply is sent until the situation has changed.
            elif bool({True for (L, F, j) in self.test_requests if F == self.core
                       or (F != self.core and L <= self.level)}):
                self.process_test_requests()

            elif self.test_over and self.find_count == 0:
                self.report()

            elif self.expect_core_report == False:
                if self.report_over == True:
